...
// {isPrivate ? <PrivateBlogPost/> : <PublicBlogPost />}
 return <>
    <h2>{props.title}</h2>
...
function PublicBlogPost(props) {
 return <div className="blog-post">
    <BlogPost key={props.title} title={props.title} author={props.author} date={props.date}/>
    <p>{props.content}</p>
  </div>
}
...
function BlogList() {
  return blogPosts.map(blogPost => {
    if (blogPost.isPrivate === true) {
      return <PrivateBlogPost key={blogPost.title} title={blogPost.title} author={blogPost.author} date={blogPost.date}/>
    }
    return <PublicBlogPost key={blogPost.title} title={blogPost.title} author={blogPost.author} date={blogPost.date} content={blogPost.content}/>
   })
  }  
...
function Footer(props) {
  return <footer>&copy; {props.year} My Blog. All rights reserved.</footer>
};
